...
make_grid <- function(grid.size,sigma,data,pr_alpha,pr_beta)
{
  ...
  marg.alpha <- rowSums(norm_post) # derive the marginal posterior
  marg.beta <- colSums(norm_post) # derive the marginal posterior
...
results_2 <- make_grid(grid.size=200,sigma=sigma,data=df,pr_alpha=c(-1.95,0.05),pr_beta=c(3.04,0.01)) # Change grid.size here to assess sensitivity

pr_alpha <- dnorm(log10(exp(seq(-8,-1,length=100))),mean=-1.95,sd=0.173,log=FALSE)
pr_alpha2 <- dnorm(log10(exp(seq(-8,-1,length=100))),mean=-1.95,sd=0.05,log=FALSE)
plot(names(results$marg.alpha),results$marg.alpha,ylab="Posterior Density",xlab=expression(alpha),type="l",lwd=2,xlim=c(-7,-2),ylim=c(0,0.25))
...
lines(seq(-8,-1,length=100),pr_alpha2/sum(pr_alpha2),lwd=2,col="orange")
legend("topright",c("Posterior (prior 1)","Posterior (prior 2)","Prior 1","Prior 2"),lwd=2,col=c("black","red","blue","orange"))

plot(names(results$marg.beta),results$marg.beta,ylab="Posterior Density",xlab=expression(beta),type="l",lwd=2,xlim=c(2.9,3.2),ylim=c(0,0.15))
...
pr_beta2 <- dnorm(seq(2.9,3.2,length=100),mean=3.04,sd=0.01,log=FALSE)
lines(seq(2.9,3.2,length=100),pr_beta2/sum(pr_beta2),lwd=2,col="orange")
...
size_grid <- 1:80
ppd <- sapply(size_grid,function(x){rnorm(length(posterior_samps_lna),mean=posterior_samps_lna+posterior_samps_beta*log(x),sd=sigma)})
