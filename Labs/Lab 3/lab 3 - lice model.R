...
#Fit model with categorical interaction between length and species
fit1<-glm(p.total~length*species,data=lice_data,family = poisson(link = "log"))
summary(fit1)

#Compare te above interaction model to a model w/o interaction
fit1b <- glm(p.total~length,data=lice_data,family = poisson(link = "log"))
summary(fit1b)
...
n_pars <- length(coefs)
#saveRDS(object=vcov(m1),file="Labs/Lab 3/var_covar_homework.rds")
#saveRDS(object=coef(m1),file="Labs/Lab 3/inits_homework.rds") #c(-0.4088346,0.0090470,0.2285305,-0.0082515)

mcmc_covar <- readRDS(file="Labs/Lab 3/var_covar_homework.rds")
beta_init <- readRDS(file="Labs/Lab 3/inits_homework.rds")
...
{
  Xvar <- data$Xvar
  prior.lambda <- -dnorm(beta[1],mean=-1e-4,sd=2,log=TRUE)
  prior.beta <- -dnorm(beta[2:ncol(Xvar)],mean=0,sd=2,log=TRUE)
  pred <- exp(Xvar %*% beta)
